...
SummarizeHub exposes five MCP tools for AI agent integration:

| Tool | Description |
|------|-------------|
| `summarize_text` | Summarize plain text |
| `summarize_image` | Caption image (BLIP) and summarize |
| `summarize_audio` | Transcribe audio (Whisper) and summarize |
...
See `skills/summarizehub/SKILL.md` for agent workflows and anti-patterns.

---

## Grading loop

The subjective grading system scores summaries on four dimensions (coherence, faithfulness, fluency, relevance) using a heuristic judge — no external API key required.

```python
from textSummarizer.grading import SummarizationLoop

loop = SummarizationLoop(model="extractive", max_iterations=2)
result = loop.run("Long source text here...")
print(result.summary, result.score.average, result.passes)
```
...
| Modality | Pipeline | Default model |
|----------|----------|---------------|
| Text | Direct summarization | `extractive` (CPU, no GPU) |
...
Install multimodal extras for image/audio: `uv sync --extra multimodal`
